"""Capture skills-styling vergelijking: text-rij vs pills, VP1 Olivier 1440x900.

Pass 2.4 deliverable. Loopt tegen een lopende Next.js dev-server op
localhost:3000. Frame staat impliciet op productie-default magazine.
"""
...
def capture(option_id: str) -> Path:
    out = OUT_DIR / f"_skills-{option_id}-vp1.png"
    with sync_playwright() as p:
        ...
        page.goto(
            f"http://localhost:3000/?skills={option_id}",
...
def assemble_grid(captures: List[Tuple[str, str, Path]]) -> Path:
    ...
    out = OUT_DIR / "puna-skills-compare-pass2-4.png"
    canvas.save(out)
