"""Capture foto-frame vergelijking: magazine vs polaroid vs layered, VP1 Olivier.

Pass 2.4 deliverable. Loopt tegen een lopende Next.js dev-server op
localhost:3000. Skills staat impliciet op productie-default pills.
"""
...
OPTIONS: List[Tuple[str, str]] = [
    ("magazine", "Magazine-frame"),
    ("polaroid", "Polaroid-cream"),
    ("layered", "Layered"),
]
...
def capture(option_id: str) -> Path:
    out = OUT_DIR / f"_frame-{option_id}-vp1.png"
...
def assemble_grid(captures: List[Tuple[str, str, Path]]) -> Path:
    # Drie cellen breed, schaal naar 1/3 horizontaal voor compactere grid.
    cell_w = VIEWPORT_W // 3       # 480
    # Behoud aspect-ratio van source 1440x900 onder de schaling.
    cell_h = int(VIEWPORT_H * (cell_w / VIEWPORT_W))  # 300
    label_h = 60
    gutter = 8
    ...
    out = OUT_DIR / "puna-frame-compare-pass2-4.png"
    canvas.save(out)
