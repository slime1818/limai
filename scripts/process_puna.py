...
# Text zone retained at its pre-heatmap-shift default (reverts experimental y=450-650).
# Under the F6 left-gradient scrim architecture (locked 2026-04-18), the 55%
# full-viewport scrim + per-biome text-zone calibration pass is deprecated.
# Gate 8 measurement is now informational only — the F6 gradient in the Next.js
# build provides local text-zone contrast architecturally, independent of painting
# brightness per-biome. Text-zone here is kept for informational heatmap continuity.
...
<head>
<meta charset="utf-8">
<title>Cross-biome style cohesion &mdash; Apu vs Puna (F6 scrim)</title>
<style>
  :root {
  ...
  * { box-sizing: border-box; }
  ...
  .gradient-overlay { position: absolute; top: 0; left: 0; right: 0;
                      height: 72vh; pointer-events: none; display: none;
  ...
  p.note { font-size: 0.82rem; opacity: 0.75; max-width: 92ch; line-height: 1.55; }
...
<body>
<h1>Cross-biome style cohesion &mdash; Apu (biome 1) vs Puna (biome 2) &middot; F6 scrim</h1>
<div class="toolbar">
  <span>View:</span>
...
<tr>
  <td class="info">p95 contrast on +55% scrim composite (legacy measurement)</td>
  <td class="num info">{{apu_p95}}:1</td>
  <td class="num info">{{puna_p95}}:1</td>
  <td class="num info">{{delta_p95}}</td>
</tr>
<tr>
  <td class="info">Worst-pixel contrast on +55% scrim (legacy diagnostic)</td>
  <td class="num info">{{apu_worst}}:1</td>
  <td class="num info">{{puna_worst}}:1</td>
  <td class="num info">{{delta_worst}}</td>
</tr>
<tr>
  <td>Production composite file size (raw, WebP q90)</td>
  <td class="num">{{apu_comp_kb}} KB</td>
  <td class="num">{{puna_comp_kb}} KB</td>
  <td class="num">{{delta_comp_kb}}%</td>
</tr>
<tr>
  <td class="info">+55% scrim composite file size (legacy preview)</td>
  <td class="num info">{{apu_scrim_kb}} KB</td>
  <td class="num info">{{puna_scrim_kb}} KB</td>
  <td class="num info">{{delta_scrim_kb}}%</td>
</tr>
</table>

<p class="caveat">
  <strong>Scrim architecture update (2026-04-18 late):</strong>
  The 55% full-viewport scrim + per-biome calibration pass is <strong>superseded</strong> by the F6 left-gradient bar (CSS layer in the Next.js build, not baked into the painting). Gate 8 measurements above are retained as informational-only continuity data &mdash; they are NOT a production pass/fail gate. Puna&apos;s earlier body-AA fail (3.59:1 on +55% scrim) is architecturally resolved: the F6 gradient supplies local text-zone contrast independent of painting brightness per biome. Puna biome 2 is production-accepted.
</p>
...
<p class="note">
  ...
  <br>Left ~55% viewport = natural text-contrast. Right ~45% viewport = painting fully visible. Text-content cross-biome lives in the left 50% viewport. Selva (TOP framing, Pad C) may require a rotated/adapted gradient variant to be decided at Selva prompt-authoring time.
</p>

<h2>Reference</h2>
<p class="note">
  Both biomes at 1024&times;768 native (fal.ai UI flux-2-pro). Framing positions per Pad C: Apu+Puna LEFT cliff/outcrop (locked); Yungas/Selva/Paracas/Pac&iacute;fico TBD at prompt-authoring. Puna sky-subject layer carries cross-biome overlap zones (top 15% Apu-handoff distant snow-topped peaks; lower-middle 15% Yungas-hint cloud-forest mist wisps). Production composite = raw painterly WebP q90; F6 gradient applied at runtime by the Next.js frontend layer in Phase 2+.
</p>
