...
export function BiomeSection({
  ...
  // renderContent prop accepteerd maar genegeerd (legacy pas 4). Dual-tree CSS
  // pattern via pointer-fine variants maakt conditional rendering overbodig.
  ...
  return (
    ...
    >
      {/* Mobile inline fallback image plus scrim (desktop gebruikt ImageStack
          fixed-position layers, die zijn hidden op coarse pointer per diagnostic 1b). */}
      ...
      <div className="absolute inset-0 z-10">
        {/* Desktop 2-viewport tree, hidden op mobile. */}
        <div className="hidden pointer-fine:block h-full">
        ...
        {/* Mobile single-viewport tree, hidden op desktop. */}
        <div className="pointer-fine:hidden h-full flex flex-col justify-center gap-6 px-8">
          <div>
            <h1 className="font-display text-warm-white text-5xl leading-tight tracking-tight">
              {biome.title}
            </h1>
            <p className="mt-4 text-base text-warm-white/90 leading-relaxed">
              {biome.tagline}
            </p>
          </div>
          {biome.viewport2Teaser !== null ? (
            <p className="text-sm text-warm-white/80 leading-relaxed max-w-sm">
              {biome.viewport2Teaser}
            </p>
          ) : null}
          {biome.ctaHref !== null && biome.ctaLabel !== null ? (
            <a
              href={biome.ctaHref}
              className="inline-block text-warm-white hover:text-andes-copper underline underline-offset-4 decoration-1 transition-colors"
            >
              {biome.ctaLabel}
            </a>
          ) : null}
